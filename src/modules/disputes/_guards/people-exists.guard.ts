...
@Injectable()
export class PeopleExistsGuard implements CanActivate {
  ...
  canActivate(route: ActivatedRouteSnapshot): Observable<boolean> | boolean {
    ...
    return this.hasPeopleInStore(disputeId);
  }

  hasPeopleInStore(disputeId: string): Observable<boolean> {
    return this.store.pipe(
      select(fromStore.selectDisputeIdOfPeopleEntries),
      tap(disputeIdInStore => {
        if (disputeIdInStore !== disputeId) {
          this.store.dispatch(new fromStore.LoadPeople({ disputeId }));
        }
      }),
